...
public class WingtipsTracer implements Tracer {
    ...
    public WingtipsTracer() {
        this(new ThreadLocalScopeManager(), PropagationRegistry.getDefault());
    }

    public WingtipsTracer(ScopeManager scopeManager, PropagationRegistry registry) {
    ...
    private class WingtipsSpanBuilder implements Tracer.SpanBuilder {
        ...
        public Span startManual() {
            if (!ignoringActiveSpan) {
                ...
                if (activeScope != null) {
                    addReference(References.CHILD_OF, activeScope.span().context());
                }
            }
            return new WingtipsSpan(builder.build(true), tags);
        }
